...
class SourceLocator(object):
    def __init__(self, recv, punit=1, eps=1e-3) -> None:
        self.recv = recv
        self.punit = 1
        self.eps = 1e-3

    def __call__(self, p, plot=True):
        r0 = np.sqrt(pow(10, p[0]/10 - self.punit/10))
        r1 = np.sqrt(pow(10, p[1]/10 - self.punit/10))
        r2 = np.sqrt(pow(10, p[2]/10 - self.punit/10))
